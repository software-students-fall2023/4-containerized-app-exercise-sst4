(() => {
  ...
  const startCamera = () => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        cam_device =
          cam_device ?? stream.getVideoTracks()[0].getSettings().deviceId;
  ...
  const setupSwapCamera = () => {
    camSwapBtn.onclick = () => {
      cam_device =
        cam_devices[(cam_devices.indexOf(cam_device) + 1) % cam_devices.length];
      startCamera();
    };
  };
  ...
  const registerButton = () => {
    ...
    registerConfirmBtn.onclick = () => {
      ...
      const dotdotdot = setInterval(() => {
        registerConfirmBtn.textContent = ".".repeat(
          (submitBtn.textContent.length % 3) + 1
        );
      }, 500);
  ...
  const setupPhoto = () => {
    ...
    backBtn.onclick = () => {
      ...
      messageContainer.textContent =
        "Result of your photo analysis will appear here.";
    };
    confirmPhoto();
    registerButton();
  };
